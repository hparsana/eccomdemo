...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      await dispatch(getUserData()); // Fetch user data
      setIsLoading(false); // Stop loader once user data is fetched
    };
    ...
    return (
      <div className="flex justify-center items-center h-screen">
        <div role="status">
          <svg
            aria-hidden="true"
            className="w-16 h-16 text-gray-300 animate-spin fill-blue-600"
            viewBox="0 0 100 101"
